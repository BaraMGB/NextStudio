...
1.  **Parameter Update:**
    ...
    *   Filter settings (Cutoff, Resonance) are read.
...
2.  **MIDI Processing:**
    ...
    *   **Note On:** Triggers `unisonOrder` number of voices.
        ...
        *   **Filter Reset:** The Ladder Filter state is reset for the new note.
...
3.  **Audio Generation:**
    ...
    *   **Parameter Update (Control Rate):** Before the sample loop, `pan` and `detuneMultiplier` are recalculated. Filter Cutoff and Resonance are updated for each voice.
    ...
    *   **Voice Rendering:**
        ...
        *   **Filter:** The oscillator output is processed by a 24dB Low Pass Ladder Filter (`juce::dsp::LadderFilter`).
        *   **Envelope:** `juce::ADSR` is used to calculate the gain for each sample.
        *   The filtered output is multiplied by the `adsrGain` and `velocity`.
...
### Oscillator
*   **Sine:** `std::sin(phase)`
*   **Triangle:** Naive generation.
*   **Saw:** Naive Sawtooth minus a PolyBLEP correction term.
*   **Square:** Naive Square plus PolyBLEP correction terms.
*   **Noise:** `random.nextFloat() * 2.0 - 1.0` (White Noise)

### Filter
Each voice has its own **24dB Low Pass Ladder Filter** (`juce::dsp::LadderFilter`). This emulates the classic Moog filter sound with a steep cutoff and characteristic resonance.
