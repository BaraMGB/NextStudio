...
EditComponent::EditComponent(te::Edit &e, EditViewState &evs, ApplicationViewState &avs, te::SelectionManager &sm,
                             juce::ApplicationCommandManager &cm)
    : m_edit(e), m_editViewState(evs), m_songEditor(m_editViewState, m_toolBar, m_timeLine), m_commandManager(cm),
      m_trackListView(m_editViewState, m_timeLine.getTimeLineID()), m_scrollbar_v(true), m_scrollbar_h(false),
      ...
      m_selectButton("select clips or automation points",
                     juce::DrawableButton::ButtonStyle::ImageOnButtonBackgroundOriginalSize),
      m_lassoSelectButton("select clips or automation points with lasso band",
                          juce::DrawableButton::ButtonStyle::ImageOnButtonBackgroundOriginalSize),
      m_timeRangeSelectButton("select time range",
                              juce::DrawableButton::ButtonStyle::ImageOnButtonBackgroundOriginalSize),
      m_splitClipButton("split clip", juce::DrawableButton::ButtonStyle::ImageOnButtonBackgroundOriginalSize),
      m_timeStretchButton("stretch tempo of clips",
                          juce::DrawableButton::ButtonStyle::ImageOnButtonBackgroundOriginalSize),
      ...
      m_deleteClipButton("delete clips", juce::DrawableButton::ButtonStyle::ImageOnButtonBackgroundOriginalSize)
{
    ...
    if (m_editViewState.m_showMasterTrack)
        addAndMakeVisible(m_masterTrackComp);
    ...
    m_selectButton.setTooltip(
        GUIHelpers::translate("select clips or automation points", m_editViewState.m_applicationState));

    m_lassoSelectButton.addListener(this);
    m_lassoSelectButton.setTooltip(
        GUIHelpers::translate("lasso select clips or automation points", m_editViewState.m_applicationState));

    m_timeRangeSelectButton.addListener(this);
    m_timeRangeSelectButton.setTooltip(GUIHelpers::translate("select clips or automation points within a time range",
                                                             m_editViewState.m_applicationState));

    m_splitClipButton.addListener(this);
    m_splitClipButton.setTooltip(
        GUIHelpers::translate("split selected clip at the cursor position", m_editViewState.m_applicationState));

    m_timeStretchButton.addListener(this);
    m_timeStretchButton.setTooltip(
        GUIHelpers::translate("apply time stretching to the selected clip", m_editViewState.m_applicationState));
...
void EditComponent::paintOverChildren(juce::Graphics &g)
{
    ...
    auto stroke = m_dragOver ? m_editViewState.m_applicationState.getPrimeColour()
                             : m_editViewState.m_applicationState.getBorderColour();
...
void EditComponent::resized()
{
    ...
    int y = getTrackListRect().getBottom();

    if (m_editViewState.m_showMasterTrack) {
        if (auto mt = m_edit.getMasterTrack()) {
            auto h = m_editViewState.m_trackHeightManager->getTrackHeight(mt, true);
            m_masterTrackComp.setBounds(0, y, getWidth(), h);
            m_masterTrackComp.resized();
            y += h;
        }
    }

    m_scrollbar_v.setBounds(getSongEditorRect().removeFromRight(20));
    m_scrollbar_v.setCurrentRange(-m_editViewState.getViewYScroll(m_timeLine.getTimeLineID()),
                                  getSongEditorRect().getHeight() / 2.0);
    m_footerbar.setBounds(getFooterRect());
    m_playhead.setBounds(getPlayHeadRect());

    // Horizontal scrollbar sits below MasterTrack (or TrackList) and above Footer
    m_scrollbar_h.setBounds(getSongEditorRect().getX(), y, getSongEditorRect().getWidth(), 20);
}
void EditComponent::updateHorizontalScrollBar()
{
    auto x1 =
        m_editViewState.getVisibleBeatRange(m_timeLine.getTimeLineID(), m_timeLine.getWidth()).getStart().inBeats();
...
void EditComponent::mouseWheelMove(const juce::MouseEvent &event, const juce::MouseWheelDetails &wheel)
{
    if (event.mods.isShiftDown()) {
        auto startBeat =
            m_editViewState.getVisibleBeatRange(m_timeLine.getTimeLineID(), m_timeLine.getWidth()).getStart().inBeats();
        auto endBeat =
            m_editViewState.getVisibleBeatRange(m_timeLine.getTimeLineID(), m_timeLine.getWidth()).getEnd().inBeats();
                    ...
                    if (trackItemNode.hasType(te::IDs::AUDIOCLIP)) {
                        if (trackItemNode.hasProperty(te::IDs::source)) {
                            ...
                            if (absoluteSourceFile.existsAsFile()) {
                                auto newRelativePath =
                                    absoluteSourceFile.getRelativePathFrom(targetTempFile.getParentDirectory());
                                trackItemNode.setProperty(te::IDs::source, newRelativePath, nullptr);
                            }
                            else {
                                GUIHelpers::log("WARNING: Source file not found during autosave: " +
                                                absoluteSourceFile.getFullPathName());
                            }
                        }
                    }
                }
...
void EditComponent::updateButtonIcons()
{
    GUIHelpers::setDrawableOnButton(m_addAudioTrackBtn, BinaryData::wavetest5_svg,
                                    m_editViewState.m_applicationState.getButtonTextColour());
    GUIHelpers::setDrawableOnButton(m_addMidiTrackBtn, BinaryData::piano5_svg,
                                    m_editViewState.m_applicationState.getButtonTextColour());
    GUIHelpers::setDrawableOnButton(m_addFolderTrackBtn, BinaryData::folderopen_svg,
                                    m_editViewState.m_applicationState.getButtonTextColour());
    GUIHelpers::setDrawableOnButton(m_expandAllBtn, BinaryData::expand_svg,
                                    m_editViewState.m_applicationState.getButtonTextColour());
    GUIHelpers::setDrawableOnButton(m_collapseAllBtn, BinaryData::collapse_svg,
                                    m_editViewState.m_applicationState.getButtonTextColour());
    GUIHelpers::setDrawableOnButton(m_selectButton, BinaryData::select_icon_svg,
                                    m_editViewState.m_applicationState.getButtonTextColour());
    GUIHelpers::setDrawableOnButton(m_lassoSelectButton, BinaryData::rubberband_svg,
                                    m_editViewState.m_applicationState.getButtonTextColour());
    GUIHelpers::setDrawableOnButton(m_timeRangeSelectButton, BinaryData::select_timerange_svg,
                                    m_editViewState.m_applicationState.getButtonTextColour());
    GUIHelpers::setDrawableOnButton(m_splitClipButton, BinaryData::split_svg,
                                    m_editViewState.m_applicationState.getButtonTextColour());
    GUIHelpers::setDrawableOnButton(m_timeStretchButton, BinaryData::time_stretch_button_svg,
                                    m_editViewState.m_applicationState.getButtonTextColour());
    GUIHelpers::setDrawableOnButton(m_reverseClipButton, BinaryData::reverse_clip_svg,
                                    m_editViewState.m_applicationState.getButtonTextColour());
    GUIHelpers::setDrawableOnButton(m_deleteClipButton, BinaryData::delete_icon_svg,
                                    m_editViewState.m_applicationState.getButtonTextColour());
}
...
void EditComponent::refreshSnapTypeDesc()
{
    ...
    m_footerbar.m_snapTypeDesc =
        m_editViewState.getSnapTypeDescription(m_editViewState.getBestSnapType(x1, x2, m_timeLine.getWidth()).level);
    m_footerbar.repaint();
}
...
void EditComponent::getAllCommands(juce::Array<juce::CommandID> &commands)
{
    juce::Array<juce::CommandID> ids{
        ...
        KeyPressCommandIDs::deleteSelectedClips, KeyPressCommandIDs::duplicateSelectedClips,
        KeyPressCommandIDs::selectAllClips,      KeyPressCommandIDs::renderSelectedTimeRangeToNewTrack,
        KeyPressCommandIDs::transposeClipUp,     KeyPressCommandIDs::transposeClipDown,
...
void EditComponent::getCommandInfo(juce::CommandID commandID, juce::ApplicationCommandInfo &result)
{
    ...
    case KeyPressCommandIDs::duplicateSelectedClips:
        ...
        result.addDefaultKeypress(juce::KeyPress::createFromDescription("d").getKeyCode(),
    ...
    case KeyPressCommandIDs::deleteSelectedClips:
        ...
        result.addDefaultKeypress(juce::KeyPress::createFromDescription("x").getKeyCode(),
                                  juce::ModifierKeys::commandModifier);
        break;
    case KeyPressCommandIDs::selectAllClips:
        result.setInfo("select all Clips", "select all Clips", "Song Editor", 0);

        result.addDefaultKeypress(juce::KeyPress::createFromDescription("a").getKeyCode(),
                                  juce::ModifierKeys::commandModifier);
        break;
    ...
    case KeyPressCommandIDs::renderSelectedTimeRangeToNewTrack:
        ...
        result.addDefaultKeypress(juce::KeyPress::createFromDescription("r").getKeyCode(),
                                  juce::ModifierKeys::commandModifier);
        break;
    ...
    case KeyPressCommandIDs::reverseClip:
        ...
        result.addDefaultKeypress(juce::KeyPress::createFromDescription("b").getKeyCode(),
                                  juce::ModifierKeys::commandModifier);
        break;
    default:
        break;
...
juce::Rectangle<int> EditComponent::getTrackListRect()
{
    ...
    area.removeFromBottom(20); // Scrollbar space

    // Reserve space for MasterTrack
    if (m_editViewState.m_showMasterTrack) {
        if (auto mt = m_edit.getMasterTrack())
            area.removeFromBottom(m_editViewState.m_trackHeightManager->getTrackHeight(mt, true));
    }
...
juce::Rectangle<int> EditComponent::getPlayHeadRect()
{
    ...
    if (m_editViewState.m_showMasterTrack) {
        if (auto mt = m_edit.getMasterTrack())
            h += m_editViewState.m_trackHeightManager->getTrackHeight(mt, true);
    }
