...
AutomationLaneHeaderComponent::AutomationLaneHeaderComponent(tracktion_engine::AutomatableParameter::Ptr ap,
                                                             EditViewState &evs)
    : m_automatableParameter(ap), m_evs(evs), m_slider(ap)
{
...
te::AutomatableParameter::Ptr AutomationLaneHeaderComponent::automatableParameter() const
{
...
TrackHeaderComponent::TrackHeaderComponent(EditViewState &evs, te::Track::Ptr t) : m_editViewState(evs), m_track(t)
{
    ...
    if (auto folderTrack = dynamic_cast<te::FolderTrack *>(m_track.get())) {
        ...
        if (folderTrack->getVolumePlugin()) {
            m_volumeKnob = std::make_unique<AutomatableSliderComponent>(
                folderTrack->getVolumePlugin()->getAutomatableParameterByID("volume"));
    ...
    if (m_track->isMasterTrack()) {
        if (auto volPlugin = m_track->edit.getMasterVolumePlugin()) {
            m_volumeKnob =
                std::make_unique<AutomatableSliderComponent>(volPlugin->getAutomatableParameterByID("master volume"));
            m_volumeKnob->setOpaque(false);
            m_volumeKnob->setTrackColour(juce::Colours::darkgrey);
            addAndMakeVisible(m_volumeKnob.get());
            m_volumeKnob->setRange(0.0f, 1.0f, 0.01f); // Master volume range 0-1 usually? Or same as audio track?
            // AudioTrack uses 0-3 range, assuming +dB. Let's use standard slider setup.
    ...
    if (auto audioTrack = dynamic_cast<te::AudioTrack *>(m_track.get())) {
        ...
        if (audioTrack->getVolumePlugin()) {
            m_volumeKnob = std::make_unique<AutomatableSliderComponent>(
                audioTrack->getVolumePlugin()->getAutomatableParameterByID("volume"));
            m_volumeKnob->setOpaque(false);
            addAndMakeVisible(m_volumeKnob.get());
...
void TrackHeaderComponent::valueTreePropertyChanged(juce::ValueTree &v, const juce::Identifier &i)
{
    ...
    if (v.hasType(te::IDs::INPUTDEVICES) || v.hasType(te::IDs::INPUTDEVICE) ||
...
void TrackHeaderComponent::showPopupMenu(tracktion_engine::Track *at)
{
    ...
    auto pencilIcon = std::unique_ptr<juce::Drawable>(
        juce::Drawable::createFromImageData(BinaryData::pencil_svg, BinaryData::pencil_svgSize));
    auto deleteIcon = std::unique_ptr<juce::Drawable>(
        juce::Drawable::createFromImageData(BinaryData::delete_icon_svg, BinaryData::delete_icon_svgSize));
                    ...
                    if (id == result) {
                        ...
                        if (targetInstance) {
                            ...
                            if (targetInstance->getTargets().contains(at->itemID)) {
                                [[maybe_unused]] auto result =
                                    targetInstance->removeTarget(at->itemID, &at->edit.getUndoManager());
                            }
                            else {
                                ...
                                [[maybe_unused]] auto result =
                                    targetInstance->setTarget(at->itemID, false, &at->edit.getUndoManager(), 0);
                            }
                        }
                        else {
...
{
    m_trackName.setColour(juce::Label::ColourIds::textColourId,
                          m_editViewState.m_applicationState.getTrackHeaderTextColour());
    ...
    if (m_isDragging) {
        ...
        if (!m_track->isMasterTrack()) {
            GUIHelpers::drawFromSvg(g, isMinimized ? BinaryData::arrowright18_svg : BinaryData::arrowdown18_svg,
                                    juce::Colour(0xff000000), {1, 6, 18, 18});
        }
        ...
        if (!isMinimized) {
            ...
            g.fillRect(
                juce::Rectangle<int>(headWidth - 1, height - strokeHeight, getWidth() - headWidth, strokeHeight));
        }
        ...
        GUIHelpers::drawFromSvg(g, icon, m_editViewState.m_applicationState.getTrackHeaderTextColour(),
                                {20, 6, 18, 18});
...
void TrackHeaderComponent::mouseDown(const juce::MouseEvent &event)
{
    ...
    if (area.contains(event.getPosition()) && !m_track->isMasterTrack()) {
    ...
    if (!event.mouseWasDraggedSinceMouseDown()) {
        if (event.mods.isRightButtonDown()) {
            ...
            if (event.mods.isCtrlDown()) {
                ...
                // Handle MasterTrack explicitly
                if (auto mt = m_track->edit.getMasterTrack()) {
                    if (m_track.get() == mt)
                        mt->state.setProperty(IDs::showLowerRange, true, nullptr);
                    else
                        mt->state.setProperty(IDs::showLowerRange, false, nullptr);
                }
            }
        }
    }
...
void TrackHeaderComponent::mouseMove(const juce::MouseEvent &e)
{
    if (!m_isResizing) {
        ...
        if (e.y > height - 10 && !isMinimized && !isFolderTrack() && !m_track->isMasterTrack()) {
            m_isHover = true;
            setMouseCursor(juce::MouseCursor::UpDownResizeCursor);
        }
        else {
...
bool TrackHeaderComponent::isInterestedInDragSource(const juce::DragAndDropTarget::SourceDetails &dragSourceDetails)
{
    if (dragSourceDetails.description == "PluginListEntry") {
        if (m_track->isMasterTrack()) {
            if (auto listbox = dynamic_cast<PluginListbox *>(dragSourceDetails.sourceComponent.get())) {
                if (auto desc = listbox->getSelectedPluginDescription()) {
    ...
    if (dragSourceDetails.description == "Track" || dragSourceDetails.description == "Instrument or Effect") {
        return true;
    }
    return false;
}
...
void TrackHeaderComponent::itemDropped(const juce::DragAndDropTarget::SourceDetails &details)
{
    ...
    if (details.description == "PluginListEntry") {
        if (auto listbox = dynamic_cast<PluginListbox *>(details.sourceComponent.get())) {
            ...
            EngineHelpers::insertPluginWithPreset(m_editViewState, getTrack(),
                                                  listbox->getSelectedPlugin(m_editViewState.m_edit));
        }
    }
    ...
    if (details.description == "Instrument or Effect") {
        if (auto lb = dynamic_cast<InstrumentEffectTable *>(details.sourceComponent.get())) {
            EngineHelpers::insertPluginWithPreset(m_editViewState, getTrack(),
                                                  lb->getSelectedPlugin(m_editViewState.m_edit));
        }
    }
...
{
    if (m_isAudioTrack || m_track->isMasterTrack()) {
        if (m_isAudioTrack) {
